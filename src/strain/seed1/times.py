...
from aces.graph import plot,fig,pl
...
for s,v in d:
	...
	for i,u in enumerate(v):
		...
		p,residuals, rank, singular_values, rcond =np.polyfit(s[filter,0],s[filter,1],1,full=True)
		if p[0]<0:continue
		cc=[j,i,p[0],s[c+20,0],s[u-20,0],len(filter),np.sqrt(residuals[0]/len(filter))]
		#print cc
		segs.append(cc)
pp=[]
for u in segs:
...
#plot([p0[1:],'Young\'s Modulous (GPa/m)'],[u0,'Probability Density'],filename='young_distribution.png')
...
#plot([p0[1:],'Drop Times'],[u0,'Probability Density'],filename='drop_times_dist.png')
...
with fig('drop_position.png',legend=False,ncol=1,figsize=(14,10)):
	...
	for i,serie in enumerate(datas):
		pl.fill_between(serie[0], 0, serie[1],label=serie[2],linewidth=0,facecolor=c[i], alpha=.5)
